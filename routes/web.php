...
use App\Http\Controllers\PropuestaEventoController;
...
Route::get('/evento/{id}', [TallerController::class, 'show'])
    ->name('taller.detalle');


// Rutas del Área Privada del Asistente y Gestión de Inscripciones
Route::middleware(['auth'])->group(function () {
    Route::get('/perfil', [InscripcionController::class, 'perfil'])
        ->name('perfil.asistente');

    Route::post('/taller/{id}/inscribir', [InscripcionController::class, 'inscribir'])
        ->name('taller.inscribir');

    Route::delete('/inscripcion/{id}/cancelar', [InscripcionController::class, 'cancelar'])
        ->name('inscripcion.cancelar');

    // Ver el formulario de edición del perfil
    Route::get('/perfil/editar', [ProfileController::class, 'edit'])
        ->name('perfil.edit');

    // Actualizar nombre y correo
    Route::patch('/perfil/actualizar', [ProfileController::class, 'update'])
        ->name('perfil.update');
    ...
    // Eliminar cuenta
    Route::delete('/perfil/eliminar', [ProfileController::class, 'destroy'])
        ->name('perfil.destroy');

    // Propuesta de nuevos eventos/talleres por usuarios logueados
    Route::get('/proponer-evento', [PropuestaEventoController::class, 'create'])
        ->name('propuestas.crear');

    Route::post('/proponer-evento', [PropuestaEventoController::class, 'store'])
        ->name('propuestas.store');
});


// Rutas del Área Privada del Organizador
Route::middleware(['auth'])->group(function () {
    ...
    // Gestión de propuestas por parte del organizador
    Route::get('/organizador/propuestas', [PropuestaEventoController::class, 'indexOrganizador'])
        ->name('organizador.propuestas');

    Route::patch('/organizador/propuestas/{propuesta}/aceptar', [PropuestaEventoController::class, 'aceptar'])
        ->name('organizador.propuestas.aceptar');

    Route::patch('/organizador/propuestas/{propuesta}/rechazar', [PropuestaEventoController::class, 'rechazar'])
        ->name('organizador.propuestas.rechazar');
});